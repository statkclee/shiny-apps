...
one_variance_UI <- function(id) {
  ...
  mainPanel <- mainPanel(width = 9,
    ...
    fluidRow(
      column(4, uiOutput(ns("results_one_variance")) ),
      column(4, plotOutput(ns("plot_one_variance"))  ),
      column(1, tableOutput(ns("table_one_variance"))))
  )
  ...
  sidebarPanel <- sidebarPanel(width = 3,
     ...
     textInput( ns("onevar_sample"), "표본", value = "0.9, -0.8, 0.1, -0.3, 0.2"),
     ...
     numericInput( ns("onevar_h0"),
                  label = NULL,
                  value = 0.1, step = 0.1, width="100px"),
     
     tags$p("2. 검정방향"),
     radioButtons(
       inputId = ns("onevar_alternative"),
       label = "대립가설",
       inline = TRUE,
       choices = c(
         "\\( \\neq \\)" = "two.sided",
...
one_variance_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    
    # 1. 텍스트 -------------------------------------
    output$results_one_variance <- renderUI({
    ...
    # 2. 그래프 ------------------------------------- 
   output$plot_one_variance <- renderPlot({  
     dat <- extract(input$onevar_sample)
     ...
     p <- data.frame(x = seq(0, qchisq(0.999, df = test$parameters, lower.tail = TRUE), 0.1)) %>% 
       ...
       stat_function( fun = dchisq, fill="gray90", args = list(df = test$parameter)) +
   ...
   # 3. 표 -------------------------------------
   output$table_one_variance <- renderTable({
