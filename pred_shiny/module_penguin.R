...
module_penguin_UI <- function(id) {
  ...
  sidebarPanel <- sidebarPanel(width = 2,
     ...
     hr(),
     sliderInput( ns("bill_length"), "부리 길이(mm)",
                  min = 30,
                  max = 60,
                  value = 45,
                  step = 1
     ),
     sliderInput( ns("bill_depth"), "부리 깊이(mm)",
                  min = 10,
                  max = 25,
                  value = 17,
                  step = 1
     ),
     sliderInput( ns("flipper_length"), "물갈퀴 길이(mm)",
                  min = 170,
                  max = 250,
                  value = 200,
                  step = 1
     ),
     sliderInput( ns("body_mass"), "체중(g)",
                  min = 2700,
                  max = 6300,
                  value = 4000,
                  step = 100
     ...
     tags$hr(style="border-color: blue;"),
     actionButton( ns("run_bttn"), "성별 예측"),
     HTML("<br/><br/>상기 입력정보를 바탕으로 펭귄 성별을 예측하고자 
          하면 '성별예측' 버튼을 클릭하여 확인하세요.")
  )
  ...
  mainPanel <- mainPanel(width = 10,
    ...
    column( width =  3,
            tags$h2("팔머 펭귄 성별 예측"),
            htmlOutput( ns("gender_predicted") )
    ),
    # 입력 정보 ---------------------
    column( width = 6,
            tags$h2("팔머 펭귄 신체 입력정보"),
            div(h4(textOutput( ns("bill_length_info"))), align = "left"),
            div(h4(textOutput( ns("bill_depth_info"))), align = "left"),
            div(h4(textOutput( ns("flipper_length_info"))), align = "left"),
            div(h4(textOutput( ns("body_mass_info"))), align = "left")
    )
  )
...
# 2. 모듈 서버 ----------------------------
module_penguin_server <- function(id) {
